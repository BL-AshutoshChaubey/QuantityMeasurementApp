...
public class QuantityMeasurementServiceImpl implements QuantityMeasurementService{
    ...
    public QuantityMeasurementServiceImpl(ConversionHistoryRepository historyRepository, ArithmeticHistoryRepository arithmeticHistoryRepository, UserRepository userRepository) {
    ...
    public MeasurementResponseDTO convert(ConversionRequestDTO request) {
        ...
        try {
            ...
            // Layer Integration: Save to DB with secure caller mapping
            User currentUser = resolveCurrentUser();
            ConversionHistory history = new ConversionHistory(request.value(), request.fromUnit(), request.toUnit(), finalResult, currentUser);
    ...
    public MeasurementResponseDTO performArithmetic(ArithmeticRequestDTO request) {
        ...
        try {
            ...
            // Save to DB with secure caller mapping
            User currentUser = resolveCurrentUser();
            ArithmeticHistory history = new ArithmeticHistory(
                    request.value1(), request.unit1(),
                    request.value2(), request.unit2(),
                    request.operation().toUpperCase(), finalResult, request.resultUnit(), currentUser
            );
